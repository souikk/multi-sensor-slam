 ...
 * @LastEditTime: 2022-08-24 17:30:04
...
namespace localization
{
    ...
    bool PreprocessDataFlow::vaildData()
    {
        ...
        if (current_cloud_time_ > unsync_imu_buff_.back().time)
        {
            ...
            unsync_imu_buff_.erase(unsync_imu_buff_.begin(), unsync_imu_buff_.end() - 2);
            if (current_cloud_time_ > unsync_gnss_buff_.back().time)
            {
                unsync_gnss_buff_.erase(unsync_gnss_buff_.begin(), unsync_gnss_buff_.end() - 2);
                return false;
            }
            return false;
        }
