 ...
 * @LastEditTime: 2022-08-31 16:33:29
 * @Description:
 */
...
namespace localization
{
    ...
    bool PreprocessDataNode::deskewPoint(CloudData::pointType &point, double point_time)
    {
        ...
        for (; j < gnss_num_; j++)
        {
            if (point_time < gnss_position_[j].time)
            {
                break;
            }
        }
        double front_scale = (gnss_position_[j].time - point_time) / (gnss_position_[j].time - gnss_position_[j - 1].time);
        double back_scale = (point_time - gnss_position_[j - 1].time) / (gnss_position_[j].time - gnss_position_[j - 1].time);
        double x = front_scale * gnss_position_[j - 1].local_E + back_scale * gnss_position_[j].local_E;
        double y = front_scale * gnss_position_[j - 1].local_N + back_scale * gnss_position_[j].local_N;
        double z = front_scale * gnss_position_[j - 1].local_U + back_scale * gnss_position_[j].local_U;
