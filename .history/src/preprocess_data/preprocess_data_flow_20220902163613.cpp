 ...
 * @LastEditTime: 2022-09-02 16:35:27
...
namespace localization
{
    ...
    bool PreprocessDataFlow::vaildData()
    {
        ...
        if (current_cloud_time_ > unsync_imu_buff_.back().time - 0.15)
        {
        ...
        if (current_cloud_time_ > unsync_gnss_buff_.back().time - 0.15)
        {
            while (current_cloud_time_ >= unsync_gnss_buff_.front().time && current_cloud_time_ < unsync_gnss_buff_.at(1).time)
            {
            }
        }
