 ...
 * @LastEditTime: 2022-08-31 16:11:00
 * @Description:
 */
...
namespace localization
{
    ...
    bool PreprocessDataNode::deskewPoint(CloudData::pointType &point, double point_time)
    {
        ...
        j = 0;
        for (; j < (int)gnss_data_.size(); j++)
        {
            if (point_time < gnss_data_[j].time)
            {
                break;
            }
        }
        double front_scale = (gnss_data_[j].time - point_time) / (gnss_data_[j].time - gnss_data_[j - 1].time);
        double back_scale = (point_time - gnss_data_[j - 1].time) / (gnss_data_[j].time - gnss_data_[j - 1].time);
        double x = front_scale * gnss_data_[j - 1].local_E + back_scale * gnss_data_[j].local_E;
        double y = front_scale * gnss_data_[j - 1].local_N + back_scale * gnss_data_[j].local_N;
        double z = front_scale * gnss_data_[j - 1].local_U + back_scale * gnss_data_[j].local_U;
