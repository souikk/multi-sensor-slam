 ...
 * @LastEditTime: 2022-08-31 15:58:17
 * @Description:
 */
...
namespace localization
{
    ...
    bool PreprocessDataNode::deskewCloud(CloudData &deskewed_cloud)
    {
        ...
        for (int i = 0; i < (int)laser_cloud_in_->points.size(); i++)
        {
            ...
            deskewPoint(this_point);
            deskewed_cloud.cloud_ptr_->push_back(this_point);
        }
        return true;
    }

    bool PreprocessDataNode::deskewPoint(CloudData::pointType &point)
    {
        //# 计算当前点到第一个时刻的imu的旋转
        double point_time = point.int j = 0;
        for (; j < imu_num_; j++)
        {
        ...
        double ratio = (point_time - imu_time_[j - 1]) / (imu_time[j] - imu_time[j - 1]);
        Eigen::Quaterniond cur_point_rot_imu = imu_rot_[j - 1].slerp(ratio, imu_rot[j]);
    }
