 ...
 * @LastEditTime: 2022-08-31 13:25:04
 * @Description:
 */
...
namespace localization
{
                                             ...
                                             CloudData deskewedCloud)
    {
        ...
        //$ 遍历每个点云
        for (int i = 0; i < (int)laserCloudIn_->points.size(); i++)
        {
            CloudData::pointType this_point;
            this_point.x = laserCloudIn_->points[i].x;
            this_point.y = laserCloudIn_->points[i].y;
            this_point.z = laserCloudIn_->points[i].z;
            this_point.intensity = laserCloudIn_->points[i].intensity;

            double range = pointDistance(this_point);
            if (range < lidar_min_range_ || range > lidar_max_range_)
                continue;

            int row_idn = laserCloudIn_->points[i].ring;
