 ...
 * @LastEditTime: 2022-08-29 15:36:33
 * @Description:
 */
...
{
    GnssPublisher::GnssPublisher(std::shared_ptr<PreprocessDataNode> node_ptr, std::string topic_name, std::string frame_id, int queue_size, int policy_history)
        : frame_id_(frame_id)
    {
        node_ptr_ = node_ptr;
        ...
        auto qos = rclcpp::QoS(
            rclcpp::QoSInitialization(
                ...
                qos_profile.depth),
            qos_profile);
        pubGnss_ = node_ptr_->create_publisher<nav_msgs::msg::Odometry>(topic_name, qos);
    }
    void GnssPublisher::publishMsg(GnssData gnss_data)
    {
        nav_msgs::msg::Odometry msg;
        msg.child_frame_id = frame_id_;
        msg.header.frame_id = "map";
        msg.pose.pose.position.x = gnss_data.longitude;
        msg.pose.pose.position.y = gnss_data.latitude;
        msg.pose.pose.position.z = gnss_data.altitude;

        msg.pose.pose.orientation.w = gnss_data.orientation.w;
        msg.pose.pose.orientation.x = gnss_data.orientation.x;
        msg.pose.pose.orientation.y = gnss_data.orientation.y;
        msg.pose.pose.orientation.z = gnss_data.orientation.z;
        pubGnss_->publish(msg);
    }
