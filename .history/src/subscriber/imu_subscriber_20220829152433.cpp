 ...
 * @LastEditTime: 2022-08-29 15:24:31
 * @Description:
 */
...
{
    ImuSubscriber::ImuSubscriber(std::shared_ptr<PreprocessDataNode> node_ptr, std::string topic_name, size_t queue_size, int policy_history)
    {
        node_ptr_ = node_ptr;
    ...
    {
        // RCLCPP_INFO(node_ptr_->get_logger(), "subscribe imu message");
        ...
        imu_data_buf_.push_back(imudata);
    }

    void ImuSubscriber::accessData(std::deque<ImuData> &imu_buff)
    {
        imu_buff.insert(imu_buff.begin(), imu_data_buf_.begin(), imu_data_buf_.end());
        imu_data_buf_.clear();
    }
}