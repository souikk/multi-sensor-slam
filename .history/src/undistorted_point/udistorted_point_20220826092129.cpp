 ...
 * @LastEditTime: 2022-08-26 09:21:28
 * @Description:
 */
...
namespace localization
{
                                  ...
                                  CloudData deskewedCloud)
    {
        ...
        for (size_t i = 0; i < imu_data.size() - 1; i++)
        {
            ...
            gyr2 = imu2lidar * gyr2;
            Eigen::Vector3d angle = (imu_data[i + 1].time - last_imu_time) * (gyr1 + gyr2) / 2;
            //! 小角度计算旋转矩阵与旋转顺序无关
            ...
            imu_time[i] = imu_data[i + 1].time;

            last_rot = imu_rot[i];
            last_imu_time = imu_time[i];
            num++;
            if (imu_data[i + 1].time > cloud_end_time)
                break;
        }
